...
import packet_types from 'dns-packet/types'
...
export default class DNSProver {
	...
	async queryWithProof<T extends packet.Answer['type']>(qtype: T, qname: string) {
...
class DNSQuery {
	...
	async queryWithProof<T extends packet.Answer['type']>(qtype: T, qname: string) {
		const response = await this.dnsQuery(qtype.toString(), qname) as any;
		...
		if(isTypedArray<'DNSKEY'>(answers) && sigs.some((sig) => sig.name === sig.data.signersName)) {
			console.log(`DNSKEY RRSET on ${answers[0].name} is self-signed; attempting to verify with a DS in parent zone`);
	...
	async verifyRRSet<T extends packet.Answer>(answers: T[], sigs: packet.Rrsig[]): Promise<ProvableAnswer<T>> {
		for(const sig of sigs) {
			...
			console.log(`Attempting to verify the ${answers[0].type} RRSET on ${answers[0].name} with RRSIG=${sig.data.keyTag}/${algorithms[sig.data.algorithm]?.name || sig.data.algorithm}`)
			const ss = new SignedSet(answers, sig);

			if(!(sig.data.algorithm in algorithms)) {
				console.log(`Skipping RRSIG=${sig.data.keyTag}/${sig.data.algorithm} on ${answers[0].type} RRSET for ${answers[0].name}: Unknown algorithm`);
				continue;
			}
			...
			for(const key of answer.records) {
				if(this.verifySignature(ss, key)) {
					console.log(`RRSIG=${sig.data.keyTag}/${algorithms[sig.data.algorithm].name} verifies the ${answers[0].type} RRSET on ${answers[0].name}`);
					proofs.push(answer);
					return {answer: ss, proofs: proofs};
				}
			}
		}
		console.log(`Could not verify the ${answers[0].type} RRSET on ${answers[0].name} with any RRSIGs`);
		throw new NoValidDnskeyError(answers);
	}
	...
	async verifyWithDS(keys: packet.Dnskey[], sigs: packet.Rrsig[]): Promise<ProvableAnswer<packet.Dnskey>> {
		...
		for(let ds of answer) {
			for(let key of keysByTag[ds.data.keyTag] || []) {
				if(this.checkDs(ds, key)) {
					console.log(`DS=${ds.data.keyTag}/${algorithms[ds.data.algorithm]?.name || ds.data.algorithm}/${digests[ds.data.digestType].name} verifies DNSKEY=${ds.data.keyTag}/${algorithms[key.data.algorithm]?.name || key.data.algorithm} on ${key.name}`);
					for(let sig of sigsByTag[ds.data.keyTag] || []) {
						const ss = new SignedSet(keys, sig);
						if(this.verifySignature(ss, key)) {
							console.log(`RRSIG=${sig.data.keyTag}/${algorithms[sig.data.algorithm].name} verifies the DNSKEY RRSET on ${keys[0].name}`);
							return {answer: ss, proofs: proofs};
						}
					}
				}
		...
		console.log(`Could not find any DS records to verify the DNSKEY RRSET on ${keys[0].name}`);
		throw new NoValidDsError(keys);
	}
	...
	verifySignature<T extends packet.Answer>(answer: SignedSet<T>, key: packet.Dnskey): boolean {
		...
		if(signatureAlgorithm === undefined) {
			console.log(`Unrecognised signature algorithm for DNSKEY=${keyTag}/${key.data.algorithm} on ${key.name}`);
			return false;
		}
	...
	checkDs(ds: packet.Ds, key: packet.Dnskey): boolean {
		...
		if(digestAlgorithm === undefined) {
			console.log(`Unrecognised digest type for DS=${ds.data.keyTag}/${ds.data.digestType}/${this.prover.algorithms[ds.data.algorithm]?.name || ds.data.algorithm} on ${ds.name}`)
			return false;
		}
